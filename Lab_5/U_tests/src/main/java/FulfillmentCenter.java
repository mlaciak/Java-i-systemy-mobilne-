...
public class FulfillmentCenter {
    ...
    public void addProduct(Item item){
        try {
            ...
            if ((item.getAmount() + getCurrentCapacity()) > getMaxCapacity()) {
                ...
                throw new IndexOutOfBoundsException("item amount is less than 1");
            } else {
    ...
    public Item search(String name){
        ...
        try {
            if(placeOfItem==-1) {
                throw new IndexOutOfBoundsException("Nie ma przedmiotu o takiej nazwie błąd funkcji search w klasie FulfillmentCenter");
            }
        }
        catch (IndexOutOfBoundsException e){
            System.out.println(e);
    ...
    public void getProduct(Item item){
        ...
        try {
            if(k==-1){
                throw new IndexOutOfBoundsException("Nie ma przedmiotu o takiej nazwie błąd funkcji getProduct w klasie FulfillmentCenter");
            }
            else {
                if((items.get(k).getAmount()-item.getAmount())<0){
                    throw new IndexOutOfBoundsException("Nie można pobrać takiej ilości, ponieważ nie ma tyle w magazynie!");
                }
    ...
    public void searchPartial(String name){
        System.out.println("Wypisanie przedmiotów, których nazwa częściowo pokrywa się z: "+name);
        for(int i=0; i<items.size();i++){
    ...
    public void summary(){
        System.out.println("Wypisanie wszystkich przedmiotów");
        if(items.size()==0){
    ...
    public Item max() {
        System.out.println("Zwraca produkt, którego jest najwięcej");
